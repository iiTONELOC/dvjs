...
/**
     * Calculates the nth number of the fibonacci sequence. And returns the recursive memoized data set.
     ...
     * ```
     */
async function recursiveMemoized(nthNumber) {
    nthNumber = normalizeNthNumber(nthNumber);
    try {
        const data = await getDataFromFiles(nthNumber);
        return JSON.stringify(filterDataSets(FILTER_BY.RECURSIVE_MEMOIZED, data));
    } catch (error) {
        return error;
    }
}

module.exports = recursiveMemoized;
