...
/**
     * Calculates the nth number of the fibonacci sequence. And returns Binet's data set.
     ...
     * ```
     */
async function binets(nthNumber) {
    nthNumber = normalizeNthNumber(nthNumber);
    try {
        const data = await getDataFromFiles(nthNumber);
        return JSON.stringify(filterDataSets(FILTER_BY.BINETS, data));
    } catch (error) {
        return error;
    }
}

module.exports = binets;
