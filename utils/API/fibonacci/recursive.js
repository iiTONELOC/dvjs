...
/**
     * Calculates the nth number of the fibonacci sequence. And returns the recursive data set.
     ...
     * ```
     */
async function recursive(nthNumber) {
    nthNumber = normalizeNthNumber(nthNumber);
    try {
        const data = await getDataFromFiles(nthNumber);
        return JSON.stringify(filterDataSets(FILTER_BY.RECURSIVE, data));
    } catch (error) {
        return error;
    }
}
